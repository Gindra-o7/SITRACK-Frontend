...
const ManageAccounts = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get("/koordinator/users", {
        params,
      });
  ...
  const validateForm = () => {
    ...
    if (newUserData.roles.length === 0)
      errors.roles = "Minimal pilih satu role";
    ...
    try {
      const payload = {
        ...
        ...(newUserData.roles.some((role) =>
          ["dosen_pembimbing", "dosen_penguji", "kaprodi"].includes(role)
        ) && { nip: newUserData.nip }),
      };
      ...
      if (error.response?.data?.errors) {
        setFormErrors(
          error.response.data.errors.reduce((acc: any, curr: any) => {
  ...
  return (
    <div className="md:p-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center md:flex-row flex-col md:justify-between mb-4 gap-2">
        ...
        <div className="hidden md:flex items-center gap-2 flex-wrap">
          {roles.map((role) => (
            <Button
              ...
              size="sm"
              className="whitespace-nowrap text-base py-1 border-gray-200"
            >
              {role.label}
            </Button>
          ))}
            ...
            >
              {roles.map((role) => (
                <Dropdown.Item
                  ...
                  className={selectedRole === role.id ? "bg-blue-50" : ""}
                >
                  <span
                    className={
      ...
      {!isLoading && !error && (
        <>
          <Table hoverable>
            <Table.Head>
              {["Nama", "Email", "Role", "Dibuat", "Aksi"].map(
                (header, index) => (
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {roles.slice(1).map((role) => (
                  <Badge
                    key={role.id}
                    color={
                      newUserData.roles.includes(role.id) ? "info" : "light"
                    }
            ...
            {(newUserData.roles.includes("mahasiswa") ||
              ...
              <div>
                ...
                <TextInput
                  ...
                  onChange={handleInputChange}
                  color={
                    formErrors.nim || formErrors.nip ? "failure" : undefined
                  }
